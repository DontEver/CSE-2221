...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber j = new NaturalNumber2(1);
        int i = 0;
        while (i < p.toInt()) {
            j.multiply(n);
            j = j.divide(m);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        boolean isWCopyEqualOne = wCopy.equals(one);
        boolean isWEqualOne = !w.equals(one);
        return (isWCopyEqualOne ? true : isWEqualOne);
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(three) <= 0) {
            isPrime = (n.equals(two) || n.equals(three));
        } else {
            for (int i = 1; i < 50; i++) {
                NaturalNumber guess = randomNumber(nCopy);
                while (guess.compareTo(one) <= 0
                        || guess.compareTo(nCopy) >= 0) {
                    guess = randomNumber(nCopy);
                }
