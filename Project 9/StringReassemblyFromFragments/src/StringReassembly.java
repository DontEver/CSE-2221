...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combined = str1.substring(0, str1.length() - overlap) + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // A flag to determine whether str should be added to strSet
        boolean shouldAddStr = true;

        Set<String> stringsToRemove = strSet.newInstance();

        for (String currentStr : strSet) {
            if (str.contains(currentStr)) {
                stringsToRemove.add(currentStr);
            } else if (currentStr.contains(str)) {
                shouldAddStr = false;
            }
        }

        if (shouldAddStr) {
            strSet.remove(stringsToRemove);
            strSet.add(str);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        while (index < text.length()) {
            ...
            // If the current character is '~', then print a new line
            if (currentChar == '~') {
                out.println();
            } else {
                // Else, print the character found
