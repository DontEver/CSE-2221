...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        // Keep guessing until the difference between the bounds is small enough
        do {
            // Choose a guess halfway between the current bounds
            guess.copyFrom(tooHigh);
            guess.add(lowEnough);
            guess.divide(two);

            // Compute the power of the guess
            power.copyFrom(guess);
            power.power(r);

            // Adjust the bounds based on whether the guess is too high or too low
            if (power.compareTo(n) <= 0) {
                // The guess is too low, so adjust the lower bound
                lowEnough.copyFrom(guess);
            } else {
                // The guess is too high, so adjust the upper bound
                tooHigh.copyFrom(guess);
            }

            // Calculate the new difference between the bounds
